...
{
  //echo "SELECT * from ".$table."  where ".$con."='".$val."' and status=1";
	$query=$CI->db->query("SELECT * from ".$table."  where ".$con."='".$val."'")->result();
...
{
$CI = &get_instance();
$max=max($start,$endlocationid);
$min=min($start,$endlocationid);
$query=$CI->db->query("SELECT locationId,locationName from location where locationId between '".$min."' AND '".$max."' order by locationId asc ")->result();
return $query;

}

function getlocation_new($start,$endlocationid,$gid)
...
$query=$CI->db->query("SELECT locationId,locationName from location where locationId between '".$min."' AND '".$max."' and groupId='".$gid."' order by locationId asc ")->result();
...
{
	$date=date('Y-m-d');
	$prev_date = date('Y-m-d', strtotime($date .' -6 day'));	
...
function rowspancount($hid,$gid,$type)
{
$CI = &get_instance();
if($type=='hq')
{
$query=$CI->db->query("SELECT * from SectionMaster where hqId='".$hid."' group by groupId ")->result();
}
else
{
$query=$CI->db->query("SELECT * from SectionMaster where hqId='".$hid."' and  groupId='".$gid."' ")->result();	
}
